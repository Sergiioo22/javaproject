...
public class NFController {
	...
	// AÑADIDO: Estado conectado tras realizar un ping exitoso al directorio
	...
	private boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_HELP:
			// Comandos permitidos en cualquier estado (offline o conectados)
			commandAllowed = true;
			break;
		case NFCommands.COM_FILELIST_DIR:
		case NFCommands.COM_PEERLIST:
		...
		case NFCommands.COM_DOWNLOAD_DIR:
		case NFCommands.COM_DOWNLOAD_PEER:
			// Para interactuar con la red, el autómata requiere haber conectado primero
			if (currentState == OFFLINE) {
				System.err.println("* ERROR: You must ping the directory first before using this command.");
	...
	private void updateCurrentState(boolean success) {
		...
		switch (currentCommand) {
		case NFCommands.COM_PING:
			// Si la operación ping finaliza con éxito, avanzamos en el autómata al estado conectado
			currentState = CONNECTED;
			break;
		default:
			// El resto de operaciones actuales no alteran este autómata básico
			break;
		}
	}
